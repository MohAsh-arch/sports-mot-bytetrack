...
def render_annotated_video(video_path, tracks_df, ball_df, poss_df,
    ...
    Returns output path.
    Uses cv2.VideoWriter (streaming, not buffered) to avoid RAM spikes.
    """
    ...
    # Use mp4v codec for cv2.VideoWriter streaming (no buffering)
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    writer = cv2.VideoWriter(str(out_path), fourcc, float(fps), (vid_w, vid_h))
    ...
    try:
        while idx < frames_to_run:
            ...
            if not ret:
                break
            ann = frame.copy()
            del frame  # free original immediately
            ...
            # Stream-write frame immediately — no buffering
            writer.write(ann)
            del ann  # free annotated frame right away
            frames_written += 1
    ...
    print(f"Video saved: {out_path} ({frames_written} frames)")
    return out_path
